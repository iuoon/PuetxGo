...
func (player *Player) Set4Mysql() error {
	...
	var str string
	if r.Get4Mysql() == nil {
		str = PtDB.GenerateUpdateSQL(player, "")
	} else {
		str = PtDB.GenerateInsertSQL(player, "")
	}

	_, err := PtDB.MysqlPool.Exec(str)
	if err != nil {
...
func (player *Player) Get4Mysql() error {
	str := PtDB.GenerateSelectSQL(player, "")
...
func (player *Player) Del4Mysql() error {
	str := PtDB.GenerateDeleteSQL(player, "")
	_, err := PtDB.MysqlPool.Exec(str)
	if err != nil {
		return err
	}
